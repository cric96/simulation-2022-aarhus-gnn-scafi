 ...
 */
public abstract class BaseDrawDensityMap extends DrawLayersValues {
     ...
     */
    protected BaseDrawDensityMap(final LayerToFunctionMapper mapper) {
        super(mapper);
    }
        ...
        for (int i1 = viewStartX; i1 < viewEndX; i1 += stepX) {
            ...
            for (int j1 = viewStartY; j1 < viewEndY; j1 += stepY) {
                ...
                if (v >= getMinLayerValueDouble()) {
                    ...
                    final double wrapperAlpha = newAlpha > getAlpha().getVal() ? getAlpha().getVal() : newAlpha;
                    final double hue =  map(v, getMinLayerValueDouble(), getMaxLayerValueDouble(), 0, 1);
                    var color = Color.getHSBColor((float)(1.0f - hue) * 0.85f - 0.15f, 1f, 1f);
                    graphics.setColor(new Color(color.getRed(), color.getGreen(), color.getBlue(), (int) wrapperAlpha));
